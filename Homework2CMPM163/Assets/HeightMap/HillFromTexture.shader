...
Shader "Custom/HillFromTexture"
{
    ...
    SubShader
    {
        Pass {
           ...
           fixed4 frag(v2f i) : SV_Target
           {
                ...
                float3 vReflect = reflect( V, i.normal );
                float4 reflectColor = texCUBE( _Cube, vReflect );
                float3 vRefract = refract( V, i.normal, 0.5 );
                ...
                //REFLECT
                float3 vRefractRed = refract( V, i.normal, 1.0 );
                float3 vRefractGreen = refract( V, i.normal, 1.0 );
                float3 vRefractBlue = refract( V, i.normal, 1.0 );
                ...
                if (i.heightVal < 0.5) {
                    ...
                    textureColor = lerp(dirt * reflectColor + refractColor, grass, i.heightVal*2);    
                }
                ...
                return float4(lerp(textureColor, lightColor, 0.5), 1.0);
                //return _Color;
        ...
        Pass {
           ...
           fixed4 frag(v2f i) : SV_Target
           {
                ...
                //FINAL LIGHT COLOR OF FRAGMENT
                float3 lightColor = float3(diffuse + specular);
    ...
    SubShader {
      ...
      void surf (Input IN, inout SurfaceOutput o) {
          o.Albedo = tex2D (_DirtTex, IN.uv_DirtTex).rgb * 0.5;
